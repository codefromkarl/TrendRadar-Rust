...
在环境准备阶段，这台机器已经补齐仓库约定的基础工具和扩展工具：
...
环境准备阶段曾使用 ContextAtlas 作为主要的上下文管理基础。当前仓库的 AI 协作规则已经切换为：先读取不可推导的项目记忆，再优先使用 GitNexus 做代码语义检索；GitNexus 未索引、索引失败或需要精确文本匹配时，再使用 `rg` 补充。

当前做法是：

- 每次会话开始，先执行一次语义代码检索
- 优先使用 GitNexus 理解仓库结构、入口、模块职责和影响面
...
当前代码索引以 GitNexus 的代码图谱与 `rg` 精确搜索组合为主：

- 语义检索：理解“这段代码是做什么的”
- 图谱检索：理解符号、调用链、路由、影响面和执行流
- 词法检索：理解“这个符号或关键字出现在哪里”

实际使用顺序是：

1. 先检查 GitNexus 是否已经索引当前仓库
2. 已索引时，优先用 GitNexus 做语义定位、上下文分析和影响面判断
3. 未索引、索引过期或需要精确文本匹配时，再用 `rg` 做补充确认
4. 对关键入口、核心模块和重要文档进行人工复核
...
- ❌ 把 `cargo clean` 当作常规验证前置步骤（会清空所有编译缓存，导致全量重编译）
- ❌ 使用 `cargo test --release`（Release 编译慢 10~100 倍，且生成独立缓存）
- ❌ 随意修改 `Cargo.toml` 依赖（会触发依赖全量重编译）
- ❌ 在仍要继续本地开发时删除 `target/` 目录（Rust 增量编译的核心）

如需清理缓存，使用 `cargo sweep --time 7` 或 `just sweep`。

如果项目已经进入封存、交接或磁盘回收阶段，可以明确执行 `cargo clean` 做全量清理；这属于维护收口动作，不应混入日常开发验证流程。
...
这份文档对应的环境准备阶段已经完成。当前仓库的整体阶段判断应以迁移收尾和后续增量演进口径为准，这里保留的是环境基线，而不是当前项目状态总览。
