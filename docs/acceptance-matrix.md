...
| `ai` | [contracts/ai.md](./contracts/ai.md) | [implementation/ai.md](./implementation/ai.md) | `cargo test -p trendradar-ai`、`cargo test -p trendradar-app ai_analysis -- --nocapture` | `cargo test -p trendradar-ai` | 已提供 `AnalysisProvider` trait、`AiAnalysis` 结构、`mock` provider 和 Markdown 渲染；`app` 已接入可选 AI 旁路 | 真实远程 provider 与更丰富输出格式留待后续阶段 |
