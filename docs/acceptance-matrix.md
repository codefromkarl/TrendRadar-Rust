...
| `schedule` | [contracts/schedule.md](./contracts/schedule.md) | [implementation/schedule.md](./implementation/schedule.md) | 先复用 `fixtures/system/config/`，下一阶段再补独立 `schedule` fixture | `cargo test -p trendradar-schedule` | 配置前置已进入 `AppConfig` | 下一步是从配置映射到决策与 crate 级测试 |
| `analyze` | [contracts/analyze.md](./contracts/analyze.md) | [implementation/analyze.md](./implementation/analyze.md) | `fixtures/system/analyze/news-ranking-input.json`、`cargo test -p trendradar-analyze` | `cargo test -p trendradar-analyze` | 输入 fixture 已准备 | 下一步是扩到过滤、聚合与更高阶排序 |
