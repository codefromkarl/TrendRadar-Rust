...
| `ai` | [contracts/ai.md](./contracts/ai.md) | [implementation/ai.md](./implementation/ai.md) | `cargo test -p trendradar-ai`、`cargo test -p trendradar-app ai_analysis -- --nocapture` | `cargo test -p trendradar-ai` | 已提供 `AnalysisProvider` trait、`AiAnalysis` 结构、`mock` provider、最小 `openai-compatible` provider 和 Markdown 渲染；`app` 已接入可选 AI 旁路 | 更多 provider、流式输出与更丰富输出格式留待后续阶段 |
| `fetch` | [contracts/fetch.md](./contracts/fetch.md) | [implementation/fetch.md](./implementation/fetch.md) | `fixtures/system/fetch/rss-rust-blog.json`、`fixtures/system/fetch/hotlist-weibo.json`、`fixtures/system/fetch/invalid-rss.json`、`fixtures/system/fetch/empty-rss.json`、`cargo test -p trendradar-fetch`（含 mockito 隔离的 HTTP adapter 测试） | `cargo test -p trendradar-fetch` | 已完成 fixture adapter 与 HTTP adapter，含 `Network`/`Http`/`ParseResponse` 错误分类；Wave 5 补齐 `with_timeout()` 可配置超时构造器；Wave 7 新增 `HotlistParser` trait + 8 实现（Generic/Weibo/Zhihu/Bilibili/Toutiao/Baidu/Pengpai/Cls）+ 工厂函数 `hotlist_parser_for()` + `HttpHotlistFetcher` parser 注入；第 1 轮扩充 `GenericHotlistParser` 兼容 `newsnow` 包装响应，并把 `douyin` / `wallstreetcn-hot` / `ifeng` / `tieba` 纳入通用解析支持 | 新平台解析器可通过实现 trait 扩展 |
| `storage` | [contracts/storage.md](./contracts/storage.md) | [implementation/storage.md](./implementation/storage.md) | `fixtures/system/storage/news-roundtrip-input.json`、`fixtures/system/storage/remote-layout-s3.json`、空仓库读取断言、`cargo test -p trendradar-storage`、`cargo test --test system remote_storage_layout_fixture_defines_minimal_s3_contract -- --nocapture` | `cargo test -p trendradar-storage` | 已完成 SQLite 最小实现、去重与空仓库边界测试；Wave 5 补齐文件持久化 `SqliteNewsRepository::open(path)` 及父目录自动创建；当前已补远程对象布局契约、系统测试骨架、`MockRemoteNewsRepository` 与 `FileObjectStoreNewsRepository` 原型，并新增基于 OpenDAL 的真实 `s3/oss` 对象仓储接入 | provider 凭证治理、更多兼容 provider 与 schema 迁移留待后续阶段 |
...
| `mcp` | 暂无独立契约，使用查询工具契约 | [implementation/mcp.md](./implementation/mcp.md) | `cargo test -p trendradar-mcp` | `cargo test -p trendradar-mcp` | 已提供独立 `trendradar-mcp` 二进制，支持 `tools/list` 和查询型 `tools/call`，不复用 CLI 输出 | 更完整的 MCP 协议兼容层和写操作工具留待后续阶段 |
| `notification` | 暂无独立契约 | 暂无独立实施文档 | `crates/notification/src/lib.rs`（tests module）、`cargo test -p trendradar-notification` | `cargo test -p trendradar-notification` | 已完成 `Notifier` trait、`WebhookNotifier`、`ConsoleNotifier`、`FeishuNotifier`、`DingTalkNotifier`、`WeComNotifier`、`SlackNotifier`、`DiscordNotifier`、`NtfyNotifier`，以及 `build_notifiers_from_specs()` 可扩展 sink 工厂；`app` 层已支持 `notification.sinks` 与旧字段兼容合并 | Telegram / 邮件 / Bark 等其他扩展渠道留待后续阶段 |
| `app` | 暂无独立契约，依赖上游模块 | [implementation/app.md](./implementation/app.md) | `crates/app/tests/config_to_bootstrap.rs`、`crates/app/tests/wave2_pipeline.rs`、`crates/app/tests/wave3_schedule_gate.rs`、`crates/app/tests/wave4_http_pipeline.rs`、`crates/app/tests/binary_smoke.rs`、`tests/system/` | `cargo test -p trendradar-app` | 已具备 fixture pipeline、HTTP config pipeline 和 CLI binary 入口；Wave 5 补齐 clap CLI、tracing 日志、配置自动发现、文件 SQLite、关键词过滤、resilient 双模式；Wave 6 补齐 `--output json/html/both`、HTML 报告、通知集成；Wave 7 补齐 `--output table/markdown` CLI 路由、多平台热榜 parser 注入、CI/CD release pipeline；当前已补齐 HTTP 混合成功/失败恢复场景、新平台 payload 跳过场景、并发抓取下多失败源与稳定 retained results 场景、复杂并发慢源组合下的输出稳定性，以及根级大输入稳定性和慢源/失败源系统恢复场景 | `app` 仍为薄编排 |
...
- 根级 `tests/system/`：当前共有 68 条系统测试，已覆盖 `config_schedule_errors`、`fetch_to_domain`、`fetch_to_analyze`、`analyze_pipeline`、`storage_to_report`、`app_pipeline_modes`，以及大输入稳定性、慢源/失败源恢复、多格式输出一致性和远程对象布局契约骨架
