...
新增 API：
- `HotlistParser` trait + 4 实现（`GenericHotlistParser` / `WeiboHotlistParser` / `ZhihuHotlistParser` / `BilibiliHotlistParser`）
...
新增 AppConfig 字段：
- `hotlist_apis[].source_type: Option<String>` — 数据源类型（`"generic"` / `"weibo"` / `"zhihu"` / `"bilibili"`，默认 `"generic"`）
